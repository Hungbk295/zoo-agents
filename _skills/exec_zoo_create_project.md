...
## Zoo Files
- [ ] `projects/<slug>/CLAUDE.md` — AUTO-GENERATED (cat từ _sources/)
- [ ] `projects/<slug>/_sources/SOUL.md` — identity, tone, scope, boundaries
- [ ] `projects/<slug>/_sources/_rules.md` — read order, task execution, Hard Gates, wrap protocol
- [ ] `projects/<slug>/_sources/tools.md` — workflow commands, dev commands (nếu có), file structure
- [ ] `projects/<slug>/AGENTS.md` — nội dung `@CLAUDE.md`
- [ ] `projects/<slug>/config.yaml` — project slug, pm_name, git_repo (nếu có)
- [ ] `projects/<slug>/tasks.json` — `{"tasks":[]}`
...
## Registration — `_sources/_zoo_rules.md`
...
2. RD của task (rd_path trong tasks.json)
...
├── .claude/commands/      # /prep, /run, /review, /wrap
├── .claude/skills/        # PM-specific auto-trigger skills
├── config.yaml, tasks.json, beads.json, AGENTS.md
├── rds/ + rds/completed/
└── output/
```

### CLAUDE.md
AUTO-GENERATED. Không sửa trực tiếp. Nội dung = output của:
```bash
cat ../../_sources/_conventions.md _sources/_rules.md _sources/SOUL.md _sources/tools.md > CLAUDE.md
```
