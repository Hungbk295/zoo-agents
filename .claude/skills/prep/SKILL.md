---
name: prep
description: "Phiên chuẩn bị trước khi thực hiện task. Thảo luận giữa user và PM, clarify scope, tighten RD."
---
...
- Ghi section `Plan` trong RD: cách làm, hướng sửa, phạm vi implementation dự kiến
- `Approach` chỉ dùng cho execution notes/triển khai thực tế sau khi bắt đầu run

### 5. Confirm & update status
- User confirm "OK, ready" → update task status → `scoped`, set `ready = true`
...
- Sau `/prep`, phần `Plan` trong RD phải đủ rõ để UI hiển thị lại cho user
